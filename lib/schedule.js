...
function sessionsFromInline(text, today) {
  ...
  for (let i = 0; i < anchors.length; i++) {
    ...
    for (let j = 0; j < slots.length; j++) {
      ...
      const activity = normalize(after.replace(DAY_RE, '').replace(/^[-–—,:;()\s]+/, '')).slice(0, 160);
...
function sessionsFromTables($, root, today) {
  ...
  root.find('table').each((_, table) => {
    ...
    for (const row of rows.slice(1)) {
      const range = normalize($(row[0]).text()).match(SLOT_RE);
      if (!range) continue;
...
function closureNotice(text) {
  const hit = /(закрыт(?:а|о|ы)?|не\s+работает|ремонт|отключени[ея]\s+воды)/i.exec(text);
  if (!hit) return null;
...
module.exports = { FACILITIES, addDays, getSchedule, todayIso };
