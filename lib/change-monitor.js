const { FACILITY_EMOJI, escapeHtml } = require('./format');
...
function formatChangeAlert(changes) {
  ...
  for (const change of changes.slice(0, 4)) {
    ...
    blocks.push(`<p>${emoji}<b><a href="${escapeHtml(facility.sourceUrl)}">${escapeHtml(facility.name)}</a></b><br />${rows.map(escapeHtml).join('<br />')}</p>`);
  }
