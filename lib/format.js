...
// Кнопка, которая перерисовала бы ту же самую карточку, приходит выключенной:
// Telegram по ней не присылает callback, и лишний editMessageText не уходит.
// disabled — это тип кнопки, поэтому callback_data у неё быть не может: «exactly
// one of the fields other than text, icon_custom_emoji_id, and style must be
// used». Точку у выбранного объекта оставляем — она читается и там, где
// выключенная кнопка внешне не отличается от обычной.
...
function navKeyboard(date, today, selected = 'all') {
  ...
  const facilityButton = (id, label) => (id === selected
    ? inactive(`• ${label}`)
    : { text: label, callback_data: `f:${id}:${date}` });
  return {
    inline_keyboard: [
      [
        { text: `‹ ${dateLabel(previous, true)}`, callback_data: `d:${previous}:${selected}` },
        date === today ? inactive('Сегодня') : { text: 'Сегодня', callback_data: `d:${today}:${selected}` },
        { text: `${dateLabel(next, true)} ›`, callback_data: `d:${next}:${selected}` },
      ],
      [
        facilityButton('all', 'Все'),
        ...FACILITIES.map(facility => facilityButton(facility.id, facility.emoji || facility.name)),
      ],
      [
